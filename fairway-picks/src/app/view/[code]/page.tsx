...
function PlayerCard({
  standing, rank, liveData, pickMap
}: {
  ...
  const rankStyle = rank === 1
    ...
    : rank === 2
      ? { color: '#c0c0c0', fontWeight: 700, fontSize: 16 }
      : rank === 3
        ? { color: '#cd7f32', fontWeight: 700, fontSize: 16 }
...
// ─── Main Guest View ──────────────────────────────────────────────────────────
export default function GuestLeaguePage({ params }: { params: { code: string } }) {
  ...
  const fetchLeagueData = useCallback(async () => {
    const res = await fetch(`/api/league-data?invite_code=${code}`)
    if (!res.ok) {
  ...
  }, [fetchLeagueData, fetchScores])

  // Auto-refresh scores every 2 minutes when tournament is active
  useEffect(() => {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, background: 'var(--bg)' }}>
        ...
        <div style={{ color: 'var(--text-dim)', fontFamily: 'DM Mono', fontSize: 13 }}>Loading league…</div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, background: 'var(--bg)', padding: 24 }}>
        ...
        <div style={{ fontFamily: 'DM Serif Display', fontSize: 24, color: 'var(--text)' }}>League not found</div>
        <div style={{ color: 'var(--text-dim)', fontSize: 14 }}>No league with code <strong style={{ color: 'var(--green)', fontFamily: 'DM Mono' }}>{code}</strong></div>
        <a href="/" style={{ marginTop: 8, color: 'var(--green)', fontSize: 13, textDecoration: 'underline' }}>← Back to Fore Picks</a>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontFamily: 'DM Mono', fontSize: 10, color: 'var(--text-dim)', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '2px 7px', letterSpacing: '0.1em' }}>
            {code}
          </span>
        </div>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '24px 16px' }}>
              ...
              <table className="table">
                <thead>
                  <tr>
                    <th>Tournament</th>
                    <th>Winner</th>
                    {Object.keys(history[0].money).map(p => <th key={p}>{p}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {history.map((h: any, i: number) => (
                    <tr key={i} className="row">
                      ...
                      <td>
                        <div style={{ fontWeight: 600, color: 'var(--gold)' }}>{h.winner ?? '—'}</div>
                      </td>
        ...
        <div style={{ textAlign: 'center', padding: '24px 0', borderTop: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href={`/join/${code}`} className="btn btn-primary" style={{ textDecoration: 'none', fontSize: 14 }}>
              Join this League
            </a>
