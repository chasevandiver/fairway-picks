...
export async function fetchLiveScores(): Promise<GolferScore[]> {
  try {
    ...
    const competitors: GolferScore[] = raw.map((c: any, idx: number) => {
      ...
      // ── Thru + in-progress detection ──
      const stats: any[] = c.statistics || []
      const thruStat = stats.find((s: any) =>
        s.name === 'thru' || s.abbreviation === 'THRU' || s.name === 'holesCompleted'
      )
      // displayValue is "17", "F", "--" etc. Fall back to numeric value as string.
      const thruStatVal = thruStat?.displayValue ?? thruStat?.value?.toString()
      // ESPN also sometimes puts thru on the status object or as period
      const statusThru = c.status?.thru
      const statusPeriod = c.status?.period
      const statusRawName = (c.status?.type?.name || '').toLowerCase()

      const isFinishedToday =
        statusRawName.includes('complete') || statusRawName.includes('final') ||
        thruStatVal === 'F' || thruStatVal === '18' ||
        String(statusThru) === '18' || String(statusPeriod) === '18'

      // Resolve the thru display string
      const thruResolved: string =
        status === 'cut' ? 'CUT' :
        status === 'wd'  ? 'WD'  :
        isFinishedToday  ? 'F'   :
        (thruStatVal && thruStatVal !== '--' && thruStatVal !== '-' && thruStatVal !== '0') ? thruStatVal :
        (statusThru !== undefined && statusThru !== null && String(statusThru) !== '0') ? String(statusThru) :
        (statusPeriod !== undefined && statusPeriod !== null && String(statusPeriod) !== '0') ? String(statusPeriod) :
        '—'

      const thru = thruResolved

      // ── Strip partial in-progress round from rounds array ──
      // If a golfer is still playing (not finished, not cut/wd), ESPN may include
      // a partial R4 linescore with strokes that aren't a real completed round.
      // We must null it out so the scorecard shows "—" instead of raw strokes.
      const isInProgress = status === 'active' && !isFinishedToday
      if (isInProgress && rounds[3] !== null) {
        // Only keep R4 if golfer has actually finished (thru = F or 18)
        // For anyone still on course, R4 strokes are partial — show as null
        rounds[3] = null
      }

      // ── Today (current round to par) ──
      // For in-progress golfers, derive from ESPN's total score minus completed rounds
      let today: number | null = null

      if (status === 'active' && !isFinishedToday && score !== null) {
        // today = total to-par minus the sum of completed rounds' to-par
        const completedRoundsPar = [rounds[0], rounds[1], rounds[2]]
          .filter((r): r is number => r !== null)
          .reduce((sum, r) => sum + (r - PAR), 0)
        // score = total to-par for all rounds including in-progress R4
        today = score - completedRoundsPar
        // Sanity check: today score should be within reasonable range
        if (Math.abs(today) > 15) today = null
      }

      // Fallback: last completed round minus par
      if (today === null) {
        let lastRoundIdx = -1
